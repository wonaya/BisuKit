...
class methylkit:
    @staticmethod
    def prep_methylkit(chr, samname, methratio, context, genomefile, randid) :
        import bisect
        ...
        context_file = open("tmp"+str(randid)+"/"+str(samname)+"-"+str(chr)+"_zed_top.out",'w')
        for a_f in open(methratio, 'r') :
            if a_f.split("\t")[0] == chrid+str(chr) and a_f.split("\t")[2] == "+" and a_f.split("\t")[3] == context :
                context_file.write(a_f)
        context_file.close()
        context_file = open("tmp"+str(randid)+"/"+str(samname)+"-"+str(chr)+"_zed_bottom.out",'w')
        for a_f in open(methratio, 'r') :
            if a_f.split("\t")[0] == chrid+str(chr) and a_f.split("\t")[2] == "-" and a_f.split("\t")[3] == context :
                context_file.write(a_f)
        context_file.close()
        ...
        for a_f in open("tmp"+str(randid)+"/"+str(samname).split(".bam")[0]+"-"+str(chr)+"_zed_top.out", 'r') : 
            large_list[int(float(a_f.split("\t")[1]))][0] += int(float(a_f.split("\t")[6]))
            large_list[int(float(a_f.split("\t")[1]))][1] += int(float(a_f.split("\t")[7]))-int(float(a_f.split("\t")[6]))
        for a_f in open("tmp"+str(randid)+"/"+str(samname).split(".bam")[0]+"-"+str(chr)+"_zed_bottom.out", 'r') : 
            large_list[int(float(a_f.split("\t")[1]))][2] += int(float(a_f.split("\t")[6]))
            large_list[int(float(a_f.split("\t")[1]))][3] += int(float(a_f.split("\t")[7]))-int(float(a_f.split("\t")[6]))
    ...
    def methylkit(file1, file2, type, chr, specie, randid) :
        ...
        robjects.r('''rm(myobj)''')
        robjects.r('''gc()''')
        robjects.r('''myDiff=calculateDiffMeth(meth, num.cores=2)''')
        robjects.r('''rm(meth)''')
        robjects.r('''gc()''')
    ...
    def eDMR(file1, file2, type, dmc, cpg, qvalue, diffmeth, randid) :
        ...
        #os.system("mv Rplots.pdf costfunction.pdf")           
...
mkgroup.add_option("--cores", dest="cores", help="no. of cores to use in running BisKit", default=16)
mkgroup.add_option("--name1", dest="bamfile1", help="name of sample 1")
mkgroup.add_option("--name2", dest="bamfile2", help="name of sample 2")
mkgroup.add_option("--zedmethratio1", dest="zedmethratio1", help="path of ZED-align methratio.txt sample1")
mkgroup.add_option("--zedmethratio2", dest="zedmethratio2", help="path of ZED-align methratio.txt sample2")
...
print options.context, options.genome, options.specie, options.cores
...
for genome in open(options.genome, 'r') :
    if genome[0] == ">" :
        if len(chr_list) == 0 :
            chr_list.append(genome.split(" dna")[0][1:].strip("\n"))
            chr_order_dict[genome.split(" dna")[0][1:].strip("\n")] = start
        else :
